...
/**
 * A factory for the component that renders the loaded page.
 */
public class ContentPanelFactory {
    ...
    public ContentPanelFactory() throws IOException {
        super();
        image = ImageIO.read(Example.class.getResourceAsStream("/mog.png"))
                .getScaledInstance(-1, 64, Image.SCALE_SMOOTH);
    }
...
//        attributes.put(TextAttribute.BIDI_EMBEDDING, null);
        ...
        final ImageGraphicAttribute imageGraphicAttribute = new ImageGraphicAttribute(image,
                ImageGraphicAttribute.ROMAN_BASELINE, 0, 63);
        for (int i = 0; i < count; i += 1) {
            attributedString.addAttribute(TextAttribute.CHAR_REPLACEMENT, imageGraphicAttribute,
                    string.length() * i + string.indexOf(replacement),
                    string.length() * i + string.indexOf(replacement) + 1);
        }
