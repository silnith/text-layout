...
//		attributes.put(TextAttribute.CHAR_REPLACEMENT, new ImageGraphicAttribute(null, ImageGraphicAttribute.ROMAN_BASELINE));
		...
		final Image image = ImageIO.read(Example.class.getResourceAsStream("/mog.png")).getScaledInstance(-1, 64, Image.SCALE_SMOOTH);
		...
		for (final LookAndFeelInfo lookAndFeelInfo : UIManager.getInstalledLookAndFeels()) {
			...
			final JMenuItem menuItem = lookAndFeelMenu.add(setLookAndFeelAction);
		}
		...
		final JMenuItem exitMenuItem = fileMenu.add(exitAction);
		...
		final JPanel view = new CustomTextPanel(attributedString);
		final JScrollPane scrollPane = new JScrollPane(view, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
